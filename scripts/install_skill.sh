...
usage() {
  cat <<'EOF'
Install Crush邂逅.skill into Claude Code / OpenClaw / QwenPaw skill directories.
...
Options:
  ...
  --skill-name <name>      Destination folder name (default: crush-xiehou-skill)
...
resolve_repo_source() {
  ...
  if [[ -d "$repo_root/Crush邂逅.skill" && -f "$repo_root/Crush邂逅.skill/SKILL.md" ]]; then
    printf '%s\n' "$repo_root/Crush邂逅.skill"
    return
  fi
...
if [[ -n "$REPO_URL" ]]; then
  ...
  SOURCE_DIR="$ROOT_DIR/Crush邂逅.skill"
fi
