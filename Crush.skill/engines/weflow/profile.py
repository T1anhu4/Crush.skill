...
def build_persona_profile(messages: List[NormalizedMessage], clusters: List[Dict[str, Any]]) -> Tuple[Dict[str, Any], str]:
    target = [m for m in messages if m.speaker == "target"]
    lengths = [len(m.content) for m in target if m.content]
    ...
    profile = {
        "safety": {
            ...
            "source": "sanitized_style_samples",
            "must_not_claim_real_person": True,
            "privacy": "no real name, wxid, avatar, phone, address, school, company, source XML",
        },
        "reply_length": {
...
def render_profile_md(profile: Dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "生成边界：只能学习表达习惯、互动节奏和聊天风格；不能声称自己是现实中的任何具体个人。",
        ]
    )
