...
Crush邂逅是一个可运行、可扩展的 **Relationship Simulation Engine（人格关系模拟引擎）**。
...
## 项目结构
- Skill 主目录: [`Crush邂逅.skill`](./Crush邂逅.skill)
...
输出文件：
- `Crush邂逅.skill/dist/crush_xiehou_openclaw.zip`
- `Crush邂逅.skill/dist/crush_xiehou_qwenpaw.zip`
...
示例：
```bash
python3 Crush邂逅.skill/execute.py --action quick_start --session-id demo --config-json '{"archetype":"experience"}'
python3 Crush邂逅.skill/execute.py --action chat_turn --session-id demo --message '你今天状态看起来不错'
python3 Crush邂逅.skill/execute.py --action postmortem --session-id demo
```
...
- 不提交：`Crush邂逅.skill/data/*.sqlite3`
